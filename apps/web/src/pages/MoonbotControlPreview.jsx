...
const client = {
  ...
  readJson: async (response) => response.endpoint?.endsWith('/cdn') ? { ok: true, enabled: false, configured: false, stale: true, targets: [] } : response.endpoint?.endsWith('/network') ? previewNetwork : ({ ok: true, configured: true, active: 'moon-primary', busy: false, observedAt: new Date().toISOString(),
    ...
    peerLatency: [{ source: 'moon-primary', configured: true, rows: [{ target: 'moon-reserve', ok: false, ms: null, at: Date.now() / 1000 }] }], canManage: true, releases: [{ id: 'alpha-demo', label: 'Alfa · versión de ejemplo', image: 'moonbot@sha256:' + 'a'.repeat(64) }],
    traffic: [{ node: 'moon-primary', ok: true, bots: [{ id: 'aabbccddeeff', name: 'Moonbot principal · ejemplo', paused: false, inflight: 1, controllable: true }, { id: '112233445566', name: 'Moonbot TDLib · ejemplo', paused: false, inflight: 0, controllable: false }] }],
