...
const client = {
  ...
  readJson: async (response) => response.endpoint?.endsWith('/cdn') ? { ok: true, enabled: false, configured: false, stale: true, targets: [] } : response.endpoint?.endsWith('/network') ? previewNetwork : ({ ok: true, configured: true, active: 'moon-primary', busy: false, observedAt: new Date().toISOString(),
    nodes: [{ id: 'moon-primary', container: 'moonbot', running: true, status: 'running', healthy: true, restarts: 1, oomKilled: false, limits: { memoryBytes: 4294967296, cpus: 2, pids: 256 } }, { id: 'moon-helper', container: 'moonbot-worker-2', running: true, status: 'running', healthy: true }, { id: 'moon-reserve', container: 'moonbot-backup', running: false, status: 'exited', healthy: false }],
    balancer: { active: true, workers: 8, processed_sources: 42, words: 128400, rate: '2150 p/min', plan: { planned_workers: 8, max_workers: 12 } },
    job: { id: 'demo-transfer', action: 'transfer-bot', node: 'moon-primary', to: 'moon-helper', status: 'completed', step: 'completed' },
    workers: [{ node: 'moon-helper', operations: { last60s: { calls: 112, received: 90 }, bots: [{ id: 'abcdef123456', last60s: { calls: 112, received: 90, sent: 36, latency_ms: 22400, errors: 0, limited: 0 } }] } }],
    tdlib: [{ node: 'moon-primary · ejemplo', configured: true, audit: { methods: 59, call_sites: 181, dynamic_calls: 5, parse_errors: 0 }, bots: [{ id: '112233445566', loaded: true, running: true, ready: true, incoming: 'bot_api', authState: 'authorizationStateReady', receiver: { events: 320, queued: 2, capacity: 4096, overflows: 0, manualStop: false } }], inbox: { enabled: true, states: { pending: 8, claimed: 1, running: 2, uncertain: 0, done: 120 }, capacity: 10000, oldestPendingSeconds: 3, workers: [{ id: 'Worker 1 · ejemplo', paused: false, running: 1, completed: 80 }, { id: 'Worker 2 · ejemplo', paused: false, running: 1, completed: 40 }] } }, { node: 'moon-helper · ejemplo', configured: false, audit: null, bots: [{ id: 'abcdef123456', incoming: 'local_bot_api_tdlib', loaded: false, ready: false }] }],
    peerLatency: [{ source: 'moon-primary', configured: true, rows: [{ target: 'moon-helper', ok: true, ms: 2.8, at: Date.now() / 1000 }, { target: 'moon-reserve', ok: false, ms: null, at: Date.now() / 1000 }] }], canManage: true, releases: [{ id: 'alpha-demo', label: 'Alfa · versión de ejemplo', image: 'moonbot@sha256:' + 'a'.repeat(64) }],
    traffic: [{ node: 'moon-helper', ok: true, bots: [{ id: 'abcdef123456', name: 'Bot de apoyo · ejemplo', paused: false, inflight: 1, controllable: true }] }, { node: 'moon-primary', ok: true, bots: [{ id: 'abcdef123456', name: 'Bot de apoyo · ejemplo', paused: true, inflight: 0, controllable: true }, { id: 'aabbccddeeff', name: 'Moonbot principal · ejemplo', paused: false, inflight: 1, controllable: true }, { id: '112233445566', name: 'Moonbot TDLib · ejemplo', paused: false, inflight: 0, controllable: false }] }],
    operations: { bots: [{ id: 'aabbccddeeff', last60s: { received: 140, calls: 210, latency_ms: 92400, sent: 60, errors: 2, limited: 1 } }, { id: '112233445566', last60s: { received: 24, calls: 38, latency_ms: 22040, sent: 12, errors: 1, limited: 1 } }, { id: 'abcdef123456', last60s: { received: 0, calls: 0, sent: 0, errors: 0, limited: 0 } }], since: new Date(minute - 7200000).toISOString(), last60s: traffic, total: { ...traffic, received: 18742, sent: 8450, calls: 28400 }, history },
