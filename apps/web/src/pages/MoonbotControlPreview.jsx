...
const client = {
  ...
  readJson: async (response) => response.endpoint?.endsWith('/cdn') ? { ok: true, enabled: false, configured: false, stale: true, targets: [] } : response.endpoint?.endsWith('/network') ? previewNetwork : ({ ok: true, configured: true, active: 'moon-primary', busy: false, observedAt: new Date().toISOString(),
    ...
    tdlib: [{ node: 'moon-primary · ejemplo', configured: true, audit: { methods: 60, call_sites: 181, dynamic_calls: 5, parse_errors: 0 }, bots: [{ id: '112233445566', loaded: true, running: true, ready: true, incoming: 'bot_api', authState: 'authorizationStateReady', receiver: { events: 320, queued: 2, capacity: 4096, overflows: 0, manualStop: false } }] }, { node: 'moon-helper · ejemplo', configured: false, audit: null, bots: [] }],
