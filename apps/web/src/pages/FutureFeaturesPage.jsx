...
const RoadmapPage = () => {
  ...
  const exportCsv = () => {
    const header = ['ID', 'Producto', 'Estado', 'Categoría', 'Prioridad', 'Título', 'Dependencia', 'Evidencia'];
    const rows = visible.map((item) => [item.id, labels[item.product], statusLabels[item.status], item.category, item.priority, item.title, item.dependency, item.evidence?.join(' | ') || '']);
  ...
  return <><Helmet><title>Roadmap | TodoSobreAllTech</title><meta name="description" content="Hoja de ruta pública de TodoSobreAllTech, Moonbot y Telegram WebApp."/></Helmet><section className="container mx-auto px-4 py-12">
    ...
    <div className="mx-auto mt-10 max-w-7xl">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-6"><Card><CardContent className="p-4"><b className="text-2xl">{catalog.total || 0}</b><p className="text-xs text-muted-foreground">Total del inventario</p></CardContent></Card><Card className="border-emerald-500/30"><CardContent className="p-4"><b className="text-2xl text-emerald-600">{catalog.implemented || 0}</b><p className="text-xs text-muted-foreground">Implementadas y verificadas</p></CardContent></Card><Card><CardContent className="p-4"><b className="text-2xl">{catalog.scaffolded || 0}</b><p className="text-xs text-muted-foreground">Con estructura parcial</p></CardContent></Card><Card><CardContent className="p-4"><b className="text-2xl">{catalog.specified || 0}</b><p className="text-xs text-muted-foreground">Solo especificadas</p></CardContent></Card><Card><CardContent className="p-4"><b className="text-2xl">{catalog.proposed || 0}</b><p className="text-xs text-muted-foreground">Sin comenzar</p></CardContent></Card><Card className="border-amber-500/30"><CardContent className="p-4"><b className="text-2xl text-amber-600">{catalog.remaining_real ?? 0}</b><p className="text-xs text-muted-foreground">Pendientes reales · {catalog.verified_percent || 0}% verificado</p></CardContent></Card></div>
