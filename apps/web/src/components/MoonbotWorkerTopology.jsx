...
export default function MoonbotWorkerTopology({ data, running }) {
  ...
  return <section className="my-5 space-y-4 rounded-2xl border bg-background p-4" aria-label="Mapa de workers Docker y sus bots">
    ...
    {(data?.tdlib || []).some(node => node.bots?.some(bot => bot.incoming === 'local_bot_api_tdlib')) && <div className="mx-auto max-w-xl rounded-xl border-2 border-violet-500/40 bg-violet-500/5 p-4 text-center"><b>Servidor oficial Bot API · transporte TDLib</b><p className="mt-1 text-sm">Telegram ↔ TDLib ↔ contrato Bot API ↔ workers</p><p className="mt-1 text-xs text-muted-foreground">{(data.tdlib || []).reduce((n, node) => n + (node.bots || []).filter(bot => bot.incoming === 'local_bot_api_tdlib').length, 0)} asignaciones de bot configuradas. Mantiene las sesiones y traduce las llamadas. La distribución de tareas entre workers requiere una cola aparte.</p></div>}
