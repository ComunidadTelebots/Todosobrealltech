...
const MoonbotGroupsManager = ({ groups = [] }) => {
  ...
  if (!selected) return <Card className="mt-8 border-violet-500/20"><CardHeader><CardTitle className="flex items-center gap-2"><UsersRound className="h-5 w-5 text-violet-600" />AdministraciÃ³n de grupos</CardTitle><CardDescription>Busca un grupo y abre su panel independiente.</CardDescription></CardHeader><CardContent><div className="relative mb-4"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre o ID" /></div><div className="grid gap-3 md:grid-cols-2">{filtered.map((group) => <button key={group.id} onClick={() => open(group)} className="rounded-xl border p-4 text-left hover:border-violet-500/50 hover:bg-muted/30"><p className="font-semibold">{group.name}</p><p className="text-xs text-muted-foreground">{group.id}</p></button>)}</div></CardContent></Card>;
