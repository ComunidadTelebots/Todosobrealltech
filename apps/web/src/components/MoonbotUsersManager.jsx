...
const MoonbotUsersManager = ({ groups = [] }) => {
  ...
  const load = async () => {
    try { const data = await call('/moonbot-admin/users'); setUsers(data.users || []); setStats(data.ban_stats || {}); } catch (cause) { setError(cause.message); }
  };
  useEffect(() => { load(); }, []);
  const filtered = useMemo(() => users.filter((user) => `${user.name} ${user.id}`.toLowerCase().includes(query.toLowerCase())), [users, query]);
  ...
  return <Card className="mt-8 border-rose-500/20"><CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-rose-600" />Usuarios y sanciones Moonbot</CardTitle><CardDescription>Consulta reputaciÃ³n, motivos y fuentes; administra baneos, notas y cuarentena.</CardDescription></CardHeader><CardContent>{error && <div className="mb-4 rounded-xl border border-red-500/30 bg-red-500/10 p-4 text-sm">{error}</div>}<div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">{Object.entries(stats).slice(0, 4).map(([key, value]) => <div key={key} className="rounded-xl border p-3"><b className="text-xl">{typeof value === 'number' ? value : 0}</b><p className="text-xs text-muted-foreground">{key.replaceAll('_', ' ')}</p></div>)}</div><div className="relative mb-4"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar usuario por nombre o ID" /></div><div className="max-h-[32rem] space-y-2 overflow-auto">{filtered.slice(0, 200).map((user) => <button key={user.id} onClick={() => open(user)} className="flex w-full items-center justify-between rounded-xl border p-3 text-left hover:bg-muted/30"><span><b>{user.name}</b><small className="ml-2 text-muted-foreground">{user.id}</small></span><span className="flex gap-2"><Badge variant="outline">{user.messages} mensajes</Badge>{user.banned && <Badge variant="destructive">Baneado</Badge>}</span></button>)}</div></CardContent></Card>;
};
