...
export default function MoonbotTelegramFlow({ data, stale }) {
  ...
  return <section className="rounded-xl border border-cyan-500/30 bg-muted/10 p-4" aria-label="Flujo de Telegram por bot">
    ...
    {!rows.length ? <p className="my-4 rounded-lg border border-dashed p-4 text-sm">{bots == null ? 'El despliegue aún no publica tráfico por bot. Actualiza Moonbot para ver las conexiones individuales.' : 'Todavía no se han observado llamadas de bots.'} API de Telegram: api.telegram.org.</p> : <>
      <div className="overflow-x-auto"><svg viewBox={`0 0 900 ${height}`} className="mt-3 w-full min-w-[650px]" role="img" aria-label="Telegram y su Bot API conectados con los bots del Docker activo; detalle numérico en la tabla">
        ...
        {rows.map((bot, index) => { const y = 65 + index * 85; const traffic = bot.last60s || {}; return <g key={bot.id}>
          <Stream path={`M450 ${middle - 8} C550 ${middle - 8} 580 ${y - 8} 680 ${y - 8}`} count={traffic.received} color="#06b6d4" running={running} />
          <Stream path={`M680 ${y + 8} C580 ${y + 8} 550 ${middle + 8} 450 ${middle + 8}`} count={traffic.calls} color="#8b5cf6" running={running} />
          <rect x="680" y={y - 30} width="205" height="62" rx="12" fill={traffic.errors ? '#f59e0b' : '#06b6d4'} opacity="0.12" /><text x="693" y={y - 8} fill="currentColor" fontSize="14">Bot · {bot.id.slice(0, 8)}</text><text x="693" y={y + 13} fill="currentColor" fontSize="11">↓ {value(traffic.received)} mensajes · ↑ {value(traffic.calls)} llamadas</text>
        </g>; })}
      </svg></div>
      <div className="overflow-x-auto"><table className="mt-3 w-full text-left text-sm"><caption className="py-2 text-left text-xs text-muted-foreground">Hasta 8 bots con más llamadas · identificadores anónimos estables por configuración · ventana de 60 s</caption><thead><tr>{['Bot', 'Recibidos', 'Peticiones API', 'Enviados', 'Errores', '429'].map(label => <th key={label} className="p-2">{label}</th>)}</tr></thead><tbody>{rows.map(bot => <tr key={bot.id} className="border-t"><td className="p-2">{bot.id.slice(0, 8)}</td>{['received', 'calls', 'sent', 'errors', 'limited'].map(key => <td key={key} className="p-2 tabular-nums">{value(bot.last60s?.[key])}</td>)}</tr>)}</tbody></table></div>
    </>}
