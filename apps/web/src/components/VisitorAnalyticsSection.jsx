...
export default function VisitorAnalyticsSection({ initialSource = 'web', selectedRange, onRangeChange }) {
  ...
  return <div className="space-y-6">
    ...
    {loading ? <p role="status">Cargando visitas registradas…</p> : error ? <p role="alert" className="rounded-xl border border-amber-500/40 bg-amber-500/10 p-4">{error}</p> : data && <>
      ...
      {!data.views && <p className="rounded-xl border p-4">{telegram ? 'Moonbot todavía no tiene idiomas registrados para mostrar.' : 'Todavía no hay visitas registradas en este periodo. Las visitas aparecerán cuando los visitantes autoricen la analítica.'}</p>}
