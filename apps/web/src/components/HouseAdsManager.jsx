...
const empty = { title: '', description: '', url: '', image: '', placement: 'all', priority: 50, enabled: true, cta: 'Abrir', background: '#eef7ff', foreground: '#155f9b', accent: '#1982d1', starts_at: '', ends_at: '', max_clicks: 0, community_id: '', community_items: [], display_format: 'auto' };
...
export default function HouseAdsManager({ groups = [] }) {
  ...
  return <section className="mt-8 rounded-xl border p-4"><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Creador de anuncios propios</h3><p className="text-sm text-muted-foreground">Diseña campañas para los huecos de NoticiasWeb3. Se muestran cuando AdSense no entrega anuncio.</p></div><Button variant="outline" onClick={recommend}><Sparkles className="mr-2 h-4 w-4"/>Recomendar campaña</Button></div>
    ...
    <div className="mb-3"><label className="text-xs text-muted-foreground">Formato no intrusivo en Telegram<select className="mt-1 h-10 w-full rounded-md border bg-background px-3 text-sm" value={draft.display_format || 'auto'} onChange={(event) => field('display_format', event.target.value)}><option value="auto">Automático discreto</option><option value="mosaic">Mosaico deslizable</option><option value="compact">Fila compacta</option><option value="cards">Tarjetas en cuadrícula</option><option value="spotlight">Una recomendación rotatoria</option><option value="ticker">Cinta de accesos</option></select></label></div>
    ...
    <div className="mt-5 grid gap-2 md:grid-cols-2">{ads.map((ad) => { const impressions = ad.impressions || 0, ctr = impressions ? ((ad.clicks || 0) * 100 / impressions).toFixed(2) : '0.00', tracking = `https://todosobreall.tech/hcgi/api/community-cards/${ad.id}/click?placement=external`; return <div key={ad.id} className="rounded-lg border p-3 text-sm"><AdPreview ad={{ ...empty, ...ad }}/><p className="mt-2 text-muted-foreground"><b>{campaignStatus(ad)}</b> · {formats[ad.placement] || ad.placement} · {impressions} impresiones · {ad.clicks || 0} clics · CTR {ctr}%{ad.max_clicks > 0 && ` · objetivo ${ad.clicks || 0}/${ad.max_clicks}`}</p>{placementSummary(ad) && <p className="mt-1 text-xs text-muted-foreground">Clics por ubicación: {placementSummary(ad)}</p>}{countrySummary(ad.clicks_by_country) && <p className="mt-1 text-xs text-muted-foreground"><b>País de los clics:</b> {countrySummary(ad.clicks_by_country)}</p>}{countrySummary(ad.impressions_by_country) && <p className="mt-1 text-xs text-muted-foreground">País de las impresiones: {countrySummary(ad.impressions_by_country)}</p>}<div className="mt-2 flex flex-wrap gap-2">{isCreator && ad.approval_status === 'pending' && <><Button size="sm" onClick={() => request({ action: 'approve', id: ad.id })}><Check className="mr-1 h-4 w-4"/>Aprobar</Button><Button size="sm" variant="destructive" onClick={() => request({ action: 'reject', id: ad.id })}><X className="mr-1 h-4 w-4"/>Rechazar</Button></>}<Button size="sm" variant="outline" onClick={() => request({ action: 'upsert', ad: { ...ad, enabled: !ad.enabled, goal_reached: false } })}>{ad.enabled ? 'Pausar' : 'Activar'}</Button><Button size="sm" variant="outline" onClick={() => { setDraft({ ...empty, ...ad }); document.getElementById('moon-house-ads')?.scrollIntoView({ behavior: 'smooth' }); }}><Pencil className="mr-1 h-4 w-4"/>Editar</Button><Button size="sm" variant="outline" onClick={() => request({ action: 'clone', id: ad.id })}><CopyPlus className="mr-1 h-4 w-4"/>Duplicar</Button><Button size="sm" variant="outline" onClick={() => setAnalyticsAd(ad)}>Estadísticas</Button><Button size="sm" variant="outline" onClick={() => request({ action: 'reset_metrics', id: ad.id })}><RotateCcw className="mr-1 h-4 w-4"/>Reiniciar métricas</Button><Button size="sm" variant="outline" onClick={() => navigator.clipboard.writeText(tracking)}><Copy className="mr-1 h-4 w-4"/>Enlace</Button><Button size="sm" variant="destructive" onClick={() => request({ action: 'delete', id: ad.id })}><Trash2 className="h-4 w-4"/></Button></div></div>; })}</div>
