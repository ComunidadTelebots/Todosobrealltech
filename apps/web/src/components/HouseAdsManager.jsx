...
import { Check, Copy, CopyPlus, Palette, Pencil, Plus, RotateCcw, Sparkles, Trash2, X } from 'lucide-react';
...
export default function HouseAdsManager({ groups = [] }) {
  ...
  const selectCommunity = (id) => {
    ...
    setDraft((current) => ({ ...current, title: current.title || group.name || '', url: username ? `https://t.me/${username}` : current.url }));
  ...
  return <section className="mt-8 rounded-xl border p-4"><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-semibold">Creador de anuncios propios</h3><p className="text-sm text-muted-foreground">Diseña campañas para los huecos de NoticiasWeb3. Se muestran cuando AdSense no entrega anuncio.</p></div><Button variant="outline" onClick={recommend}><Sparkles className="mr-2 h-4 w-4"/>Recomendar campaña</Button></div>
    ...
    <div className="mt-5 grid gap-2 md:grid-cols-2">{ads.map((ad) => { const impressions = ad.impressions || 0, ctr = impressions ? ((ad.clicks || 0) * 100 / impressions).toFixed(2) : '0.00', tracking = `https://todosobreall.tech/hcgi/api/house-ads/${ad.id}/click?placement=external`; return <div key={ad.id} className="rounded-lg border p-3 text-sm"><AdPreview ad={{ ...empty, ...ad }}/><p className="mt-2 text-muted-foreground"><b>{campaignStatus(ad)}</b> · {formats[ad.placement] || ad.placement} · {impressions} impresiones · {ad.clicks || 0} clics · CTR {ctr}%{ad.max_clicks > 0 && ` · objetivo ${ad.clicks || 0}/${ad.max_clicks}`}</p>{placementSummary(ad) && <p className="mt-1 text-xs text-muted-foreground">Clics por ubicación: {placementSummary(ad)}</p>}<div className="mt-2 flex flex-wrap gap-2">{isCreator && ad.approval_status === 'pending' && <><Button size="sm" onClick={() => request({ action: 'approve', id: ad.id })}><Check className="mr-1 h-4 w-4"/>Aprobar</Button><Button size="sm" variant="destructive" onClick={() => request({ action: 'reject', id: ad.id })}><X className="mr-1 h-4 w-4"/>Rechazar</Button></>}<Button size="sm" variant="outline" onClick={() => request({ action: 'upsert', ad: { ...ad, enabled: !ad.enabled, goal_reached: false } })}>{ad.enabled ? 'Pausar' : 'Activar'}</Button><Button size="sm" variant="outline" onClick={() => { setDraft({ ...empty, ...ad }); document.getElementById('moon-house-ads')?.scrollIntoView({ behavior: 'smooth' }); }}><Pencil className="mr-1 h-4 w-4"/>Editar</Button><Button size="sm" variant="outline" onClick={() => request({ action: 'clone', id: ad.id })}><CopyPlus className="mr-1 h-4 w-4"/>Duplicar</Button><Button size="sm" variant="outline" onClick={() => request({ action: 'reset_metrics', id: ad.id })}><RotateCcw className="mr-1 h-4 w-4"/>Métricas</Button><Button size="sm" variant="outline" onClick={() => navigator.clipboard.writeText(tracking)}><Copy className="mr-1 h-4 w-4"/>Enlace</Button><Button size="sm" variant="destructive" onClick={() => request({ action: 'delete', id: ad.id })}><Trash2 className="h-4 w-4"/></Button></div></div>; })}</div>
  </section>;
}
