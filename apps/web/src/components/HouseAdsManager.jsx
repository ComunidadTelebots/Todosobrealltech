...
export default function HouseAdsManager() {
  ...
  return <section className="mt-8 rounded-xl border p-4"><div className="mb-4"><h3 className="font-semibold">Anuncios propios de la red</h3><p className="text-sm text-muted-foreground">Sustituyen el hueco únicamente cuando AdSense no entrega anuncio.</p></div>
    ...
    <div className="mt-4 grid gap-2 md:grid-cols-2">{ads.map((ad) => { const impressions = ad.impressions || 0, ctr = impressions ? ((ad.clicks || 0) * 100 / impressions).toFixed(2) : '0.00', tracking = `https://todosobreall.tech/hcgi/api/house-ads/${ad.id}/click?placement=external`; return <div key={ad.id} className="rounded-lg border p-3 text-sm"><b>{ad.title}</b><p className="text-muted-foreground">{ad.placement} · {impressions} impresiones · {ad.clicks || 0} clics · CTR {ctr}%</p><button className="mt-1 block max-w-full truncate font-mono text-xs text-cyan-600" onClick={() => navigator.clipboard.writeText(tracking)} title="Copiar enlace medido">{tracking}</button><div className="mt-2 flex gap-2"><Button size="sm" variant="outline" onClick={() => request({ action: 'upsert', ad: { ...ad, enabled: !ad.enabled } })}>{ad.enabled ? 'Pausar' : 'Activar'}</Button><Button size="sm" variant="outline" onClick={() => navigator.clipboard.writeText(tracking)}>Copiar enlace</Button><Button size="sm" variant="destructive" onClick={() => request({ action: 'delete', id: ad.id })}><Trash2 className="h-4 w-4"/></Button></div></div>; })}</div>
  </section>;
}
