...
import { Check, Clock3, Copy, CopyPlus, Eye, Flag, Palette, Pencil, Plus, Radio, RotateCcw, Search, Sparkles, Trash2, Users, X } from 'lucide-react';
...
const empty = { title: '', description: '', url: '', image: '', placement: 'all', placements: ['all'], allowed_sites: ['all'], priority: 50, enabled: true, cta: 'Abrir', background: '#eef7ff', foreground: '#155f9b', accent: '#1982d1', starts_at: '', ends_at: '', max_clicks: 0, max_impressions: 0, display_seconds: 15, frequency_cap: 3, destination_mode: 'single', community_id: '', community_items: [], display_format: 'auto', relationship_type: 'affiliate', telegram_verified: false, community_verified: false };
...
function CampaignDeliveryControls({ draft, field, toggleChoice }) {
  return <div className="mb-4 grid gap-4 rounded-xl border bg-background p-4 lg:grid-cols-2">
    ...
    <div className="grid gap-2 sm:grid-cols-2"><label className="text-xs text-muted-foreground">Duración visible (segundos)<Input type="number" min="3" max="300" value={draft.display_seconds} onChange={(event) => field('display_seconds', Number(event.target.value))}/></label><label className="text-xs text-muted-foreground">Máximo por sesión y usuario<Input type="number" min="0" max="100" value={draft.frequency_cap} onChange={(event) => field('frequency_cap', Number(event.target.value))}/></label></div>
...
export default function HouseAdsManager({ groups = [] }) {
  ...
  useEffect(() => { loadReports().catch(() => {}); }, [isCreator]);
  ...
  const selectCommunity = (id) => {
    ...
    setDraft((current) => ({ ...current, title: group.name || current.title, description: current.description || `Únete a ${group.name || 'nuestra comunidad'} en Telegram.`, url: username ? `https://t.me/${username}` : current.url, image: photo || current.image, cta: group.ctype === 'channel' ? 'Seguir canal' : 'Unirme al grupo', destination_mode: 'single', community_id: '', community_items: [] }));
