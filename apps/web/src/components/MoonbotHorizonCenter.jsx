...
const MoonbotHorizonCenter = () => {
  ...
  useEffect(() => { load(); }, []);
  const unified = useMemo(() => data.features.map((item) => ({ ...item, id: item.id || `operational-${item.slug}`, horizonStatus: item.status || 'operational', executable: true })), [data.features]);
  ...
  const execute = async () => {
    ...
    try { const response = await apiServerClient.fetch(`/moonbot-admin/horizon/${encodeURIComponent(selected.slug)}`, { method: 'POST', headers: { Authorization: `Bearer ${pb.authStore.token}`, 'Content-Type': 'application/json' }, body: JSON.stringify(parsed) }); const body = await response.json(); if (!response.ok || !body.ok) throw new Error(body.error || `HTTP ${response.status}`); setResult(body.result); await load(); }
  ...
  return <Card className="mt-8 border-violet-500/20"><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-violet-600"/>Horizonte unificado</CardTitle><CardDescription>Las 1.100 funciones de la web, Moonbot y la Mini App reunidas en un catálogo ejecutable, configurable y auditado.</CardDescription></CardHeader><CardContent className="space-y-4">
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><div className="rounded-xl border p-3"><b className="text-2xl">{unified.length}</b><p className="text-xs text-muted-foreground">Funciones totales</p></div><div className="rounded-xl border p-3"><b className="text-2xl">{unified.filter((item) => item.resource || item.engine !== 'horizon1000').length}</b><p className="text-xs text-muted-foreground">Rutas disponibles</p></div><div className="rounded-xl border p-3"><b className="text-2xl">{categories.length}</b><p className="text-xs text-muted-foreground">Áreas</p></div><div className="rounded-xl border p-3"><b className="text-2xl">{unified.filter((item) => item.horizonStatus === 'routed').length}</b><p className="text-xs text-muted-foreground">En integración real</p></div></div>
