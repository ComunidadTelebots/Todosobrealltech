...
const MoonbotHorizonCenter = () => {
  ...
  const [roadmap, setRoadmap] = useState({ items: [], total: 0, implemented: 0 });
  ...
  const load = async () => { setBusy(true); try { const [operational, future] = await Promise.all([request(), fetch('/future-features-1000.json').then((response) => { if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.json(); })]); setData(operational); setRoadmap(future); setError(''); } catch (cause) { setError(cause.message); } finally { setBusy(false); } };
  useEffect(() => { load(); }, []);
  const unified = useMemo(() => [
    ...data.features.map((item) => ({ ...item, id: `operational-${item.slug}`, horizonStatus: 'operational', executable: true })),
    ...(roadmap.items || []).map((item) => ({ ...item, slug: item.id, horizonStatus: item.status === 'implemented' ? 'implemented' : 'proposed', executable: false })),
  ], [data.features, roadmap.items]);
  ...
  return <Card className="mt-8 border-violet-500/20"><CardHeader><CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-violet-600"/>Horizonte unificado</CardTitle><CardDescription>Un solo catálogo con las 100 funciones ejecutables y las 1.000 iniciativas compartidas por la web, Moonbot y la Mini App.</CardDescription></CardHeader><CardContent className="space-y-4">
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4"><div className="rounded-xl border p-3"><b className="text-2xl">{unified.length}</b><p className="text-xs text-muted-foreground">Funciones totales</p></div><div className="rounded-xl border p-3"><b className="text-2xl">{data.total || data.features.length}</b><p className="text-xs text-muted-foreground">Ejecutables aquí</p></div><div className="rounded-xl border p-3"><b className="text-2xl">{roadmap.implemented || 0}</b><p className="text-xs text-muted-foreground">Implementadas del roadmap</p></div><div className="rounded-xl border p-3"><b className="text-2xl">{(roadmap.total || 0) - (roadmap.implemented || 0)}</b><p className="text-xs text-muted-foreground">Pendientes</p></div></div>
