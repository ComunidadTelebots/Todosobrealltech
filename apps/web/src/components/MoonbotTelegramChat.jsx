...
import { Ban, MessageCircle, RefreshCw, Search, Send, ShieldAlert, Star, Undo2, VolumeX } from 'lucide-react';
...
const MoonbotTelegramChat = ({ bots = [] }) => {
  ...
  const send = async () => {
    ...
    try {
      await call(`/moonbot-admin/groups/${selected.id}`, { method: 'POST', body: JSON.stringify({ action: 'send_message', text: message, markdown: true, bot_id: senderBotId }) });
