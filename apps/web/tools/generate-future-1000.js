...
for (let number = 31; number <= 334; number += 1) {
...
for (let number = 1001; number <= 1196; number += 1) {
  implementedEvidence.set(`future-${String(number).padStart(4, '0')}`, [
    'moon-multibot:core/feature_runtime.py',
    'moon-multibot:tests/test_feature_runtime.py',
    'apps/api/src/routes/moonbot-admin.js',
...
const verifiedWebappIds = [
  ...
  ...Array.from({ length: 8 }, (_, index) => 993 + index),
  ...Array.from({ length: 254 }, (_, index) => 1668 + index).filter((number) => ![1701, 1710].includes(number)),
];
...
const trackedTasks = [
  ...
  ].map(([id, title, detail, evidence]) => ({ id, title, products: ['web', 'moonbot', 'webapp'], status: 'implemented', detail, evidence })),
